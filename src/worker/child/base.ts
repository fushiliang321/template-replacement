...
export default async function _init(replace: new (core: rawCoreInterface) => ReplaceInterface, core: (buffer: BufferSource) => rawCoreInterface, isSign: boolean = false) {
  const wasmUrl = await call<string>('getWasmUrl')
  const response = await fetch(wasmUrl)
  const arrayBuffer = await response.arrayBuffer()
  const signFun = isSign ? ((data: unknown): Promise<string> => {
    return call<string>('sign', data)
  }) : undefined
  dispatch = new agency(common(arrayBuffer, replace, core, signFun))
  initResolve?.()
...
addEventListener('message', async (event) => {
  ...
  switch (data.type) {
    case messageTypes.methodCall:
      ...
      await awaitInit
      const fun = dispatch[method] as ((...args: unknown[]) => unknown) | undefined
      if (!fun) {
        return
      }
      let result: unknown
